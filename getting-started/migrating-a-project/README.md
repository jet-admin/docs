...
{% hint style="info" %}
If you're rebuilding a prototype from another no-code app builder on your real data, start with Quickstart and connect your database directly. There's nothing to export.
{% endhint %}
...
If your source isn't listed, any of the 50+ data sources works the same way, and anything with an API can be connected as a REST API or GraphQL resource.
...
For example, sync Salesforce and Stripe and blend accounts with invoices in one table. Build a table view and detail view, then add an agent so your team can ask questions about the blended data in plain language.
...
Yes, with sync connections on both, using 360 Data / Data Blending. The result is read-only.
...
Yes, if it has an API. Connect it as a REST API or GraphQL resource. Ask AI can generate the requests from a curl command or a link to the API's documentation.
